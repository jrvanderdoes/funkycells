test_that("PCA Function - All Classes, people, pca present", {
  ...
  # All People computed
  expect_equal(c(1:14,16:18,20:21,23,27:29,31:37,39:41), dataPCA_pheno$Person)
...
test_that("PCA Function - Check values", {
  dataPCA_pheno1 <- getPCAData(data = TNBC_pheno, unit='Person',
                               agents_df=data.frame('B','B'),
                               nPCs = 1,
                               rCheckVals = seq(0,50,1), silent = TRUE)
  dataPCA_pheno3 <- getPCAData(data = TNBC_pheno, unit='Person',
                               agents_df=data.frame('B','B'),
                               nPCs = 3,
                               rCheckVals = seq(0,50,1), silent = TRUE)

  # Only one PCA
  expect_equal(c(-239796.36, -384747.62, -279741.44, -305658.81, -384747.62,
  ...
  expect_equal(c(-239319.79, -384168.52, -279233.51, -305145.18, -384168.52,
