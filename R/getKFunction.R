...
getKFunction <- function(data, agents, unit,
  ...
  for(i in 1:length(units)){
    ...
    for(j in 1:length(repeats)){
      ...
      data_ppp <- as.ppp(data_repeat, W=as.owin(c(xRange_rm,yRange_rm)))
...
.alignKr <- function(K, newK, r, newr){
  ...
  K_ret <- merge(data.frame('r'=bigr),
               data.frame('r'=r,K), 
               by=c("r"), all.x=TRUE)
  K_ret <- K_ret %>% fill (-r,.direction='down')
  
  newK_ret <- merge(data.frame('r'=bigr),
                data.frame('r'=newr,'K'=newK), 
                by=c("r"), all.x=TRUE)
  newK_ret <- newK_ret %>% fill (K,.direction='down')
...
.rK2DF <- function(K_list, r_list){
  ...
  data_ret <- data_ret %>% fill (-r,.direction='down')
