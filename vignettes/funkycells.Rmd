...
The variable importance plot is returned.
```{r variableimportance, eval=full_run}
model_fc$viPlot
```

The results of the model from `funkyModel()` also includes the model from `funkyModel()` on the original data, which is necessary when trying to predict new data. Suppose we create a few new people (i.e. get more data) and wish to predict. This would be done as follows.
