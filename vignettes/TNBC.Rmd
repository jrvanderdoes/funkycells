...
paperRun <- FALSE # This is on all chucks that run for paper results
#   For some reason paper results are deleted after vignette builds..
...
interactions (`r nrow(pheno_interactions_subset)`) of particular interest. 
Another analysis on the subset interaction set has been considered but was 
...
kf <- getKFunction(data=TNBC_pheno[TNBC_pheno$Person==2,-1],
             agents = c('Tumour','MonoNeu'),
             unit='Person',
             rCheckVals = seq(0,50,1))

k_fig <- ggplot2::ggplot() +
  ggplot2::geom_line(ggplot2::aes(x=r,y=K1),data=kf, size=2) +
...
png("./paper/tnbc_person1_tumour_mononeu_k.png", width = 800, height = 800)
#png("./vignettes/paper/tnbc_person1_tumour_mononeu_k.png", width = 800, height = 800)
print(k_fig)
dev.off()
```
...
plotPP(data = TNBC_pheno[TNBC_pheno$Person==2,c('cellx','celly',"Phenotype")],
...
sim_image <- plotPP(
  data = dat[dat$Image==1,c('x','y','cellType')],
  ptSize = 3, dropAxes = T, colorGuide = 'none')
...
```{r plot_noeffect_full, fig.width=5, fig.height=5, fig.cap="No Effect Variable Importance (All)"}
rfcv$viPlot
```
```{r plot_noeffect_top25, fig.width=5, fig.height=5, fig.cap="No Effect Variable Importance (Top 25)"}
rfcv$subset_viplot
```
...
```{r plot_effect_full, fig.width=5, fig.height=5, fig.cap="Effect Simulation Variable Importance (All)"}
rfcv$viPlot
```
```{r plot_effect_top25, fig.width=5, fig.height=5, fig.cap="Effect Simulation Variable Importance (Top 25)"}
rfcv$subset_viPlot
```
...
```{r tnbc_pheno_model}
dataPCA_pheno <- getKsPCAData(data = TNBC_pheno, unit='Person',
...
Creating the following variable importance plots.
```{r tnbc_full, fig.width=5, fig.height=5, fig.cap="TNBC Phenotype Variable Importance (All)"}
rfcv$viPlot
```
```{r tnbc_top25, fig.width=5, fig.height=5, fig.cap="TNBC Phenotype Variable Importance (Top 25)"}
rfcv$subset_viPlot
```
...
plot_top25 <- ggplot2::ggplot(
  ...
  ggplot2::ylab(NULL) +
  ggplot2::theme_bw() +
  ggplot2::theme(axis.text.x=ggplot2::element_text(size=17), 
...
return_plot <-
  ...
  ggplot2::geom_line(alpha=0.2, size=1.25) +
