...
on a subset of phenotypes (`r length(phenos_subset)`) and interactions (`r nrow(pheno_interactions_subset)`) of particular interest. For computational 
...
patients. We generate $15$ for each class with possible interactions, if present, 
and $2$ for each class that are indistinguishable to add some noise to the data.
