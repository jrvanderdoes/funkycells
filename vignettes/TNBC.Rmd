...
tnbc_fig <- plotPP(TNBC_pheno[TNBC_pheno$Person == 2, c("cellx", "celly", "Phenotype")],
  ptSize = 3, colorGuide = "none", dropAxes = F,
  xlab='X (\U03BCm)',ylab='Y (\U03BCm)'
)
...
k_fig <- ggplot2::ggplot() +
  ggplot2::geom_line(ggplot2::aes(x = r, y = K1, 
                                  color='K Function', linetype = "K Function"), 
                     data = kf, linewidth = 2) +
  ggplot2::geom_line(ggplot2::aes(x = seq(0, 50, 1), y = pi * seq(0, 50, 1)^2, 
                                  color='Reference', linetype = "Reference"),
                     linewidth = 2
  ) +
  ggplot2::theme_bw() +
  ggplot2::theme(
  ...
  ggplot2::xlab('r (\U03BCm)') +
  ggplot2::ylab('K') +
  ggplot2::scale_color_manual(name = NULL, 
                              values = c("K Function" = "black", 
                                         "Reference" = "red")) +
  ggplot2::scale_linetype_manual(name = NULL, 
                              values = c("K Function" = "solid", 
                                         "Reference" = "dashed")) +
  ggplot2::theme(
    legend.position = c(0.05,0.95), # top left position
...
# Same plot, but different parameters for another figure
k_fig1 <- ggplot2::ggplot() +
  ggplot2::geom_line(ggplot2::aes(x = r, y = K1, 
                                  color='K Function', linetype = "K Function"), 
                     data = kf, linewidth = 3) +
  ggplot2::geom_line(ggplot2::aes(x = seq(0, 50, 1), y = pi * seq(0, 50, 1)^2, 
...
png(paste0(save_path,"/paper/tnbc_person2_tumor_mononeu_k_simple.png"), 
    width = 800, height = 800)
print(k_fig1)
dev.off()
```
...
plot_dat <- dat[dat$replicate == 18, c("x", "y", "type")]
plot_dat$x <- plot_dat$x *2000
plot_dat$y <- plot_dat$y *2000
sim_image <- plotPP(
  data = plot_dat,
  ptSize = 3, dropAxes = T, colorGuide = "none",
...
plot_full <- ggplot2::ggplot(
  ...
  #   atop(bold("Standardized Variable Importance"),
  #        italic(sub_val))))
...
return_plot <-
  ...
  ggplot2::xlab('r (\U03BCm)') +
  ggplot2::scale_colour_hue(name = NULL,labels=c('Mixed','Compartmentalized')) +
  ggplot2::theme(
    legend.position = c(0.05,0.95), # top left position
    legend.justification = c(0.05, 0.95), # top left justification
    legend.box.margin = ggplot2::margin(5, l = 5, unit = "mm") # small margin
  ) +
  ggplot2::annotate(geom="text", x=15, y=53000, label='* Bold are the respective means',
                    size=5) +
...
return_plot <-
  ...
  ggplot2::annotate(geom="text", x=15, y=73000, label='* Bold are the respective means',
                    size=5) +
ggplot2::geom_line(ggplot2::aes(x = r, y = pi * r^2,
                                group = outcome),
                   data = data_avg, linewidth = 1.25,
