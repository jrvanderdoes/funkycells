...
#   Point estimates (real data + 1 fake)
#   Curve is permuted data + 1 fake
...
funkyModel1 <- function(data, K = 10,
  ...
  for(i in 1:15){
    data[[paste0('New_New_PC1',i)]] <- sample(data$A_A_PC1)
    data[[paste0('New_New_PC2',i)]] <- sample(data$B_B_PC2)
    data[[paste0('New_New_PC3',i)]] <- sample(data$C_C_PC3)
    #metaNames <- c(metaNames,paste0('newTest',i))
  }
