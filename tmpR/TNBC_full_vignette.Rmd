...
# Don't check as running is intensive
...
We consider here here there are `r length(phenos)` total phenotypes with `r nrow(pheno_interactions)` total interactions. Alternatively, we can also focus
on a subset of phenotypes (`r length(phenos_interest)`) and interactions (`r nrow(pheno_interactions_interest)`) of particular interest. For computational
time, we only consider the subset of data below.
...
patients. We generate $15$ for each class with possible interactions, if present, and $2$ for each class that are indistinguishable to add some noise to the data.
...
```{r compareSimAndTNBC, fig.width=5, fig.height=5, fig.cap="Compare Images"}
...
```{r examineNoEffect, fig.width=5, fig.height=5, fig.cap="No Effect All Variable Importances"}
rfcv$viPlot
```
```{r examineNoEffect, fig.width=5, fig.height=5, fig.cap="No Effect Top 25 Variable Importances"}
rfcv$subset_viPlot
```
...
```{r plotEffectVI, fig.width=5, fig.height=5, fig.cap="Effect Simulation All Variable Importance"}
rfcv$viPlot
```
```{r plotEffectVI, fig.width=5, fig.height=5, fig.cap="Effect Simulation Top 25 Variable Importance"}
rfcv$subset_viPlot
```
...
Creating the variable importance plots-both full and subsetted--as follows.
```{r viewFullPlot, fig.width=5, fig.height=5, fig.cap="TNBC Phenotype Variable Importance for All Variables"}
rfcv$viPlot
```
...
Likwise, let us consider $K$ functions from significant and insignificant interactions.
```{r significantTNBCK}
