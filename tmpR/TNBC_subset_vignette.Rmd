...
domain knowledge. Another analysis on the entire interaction set has been 
...
for(person in unique(TNBC_pheno$Person)){
  tmp <- as.data.frame(table(TNBC_pheno[TC_pheno$Person==person,"Phenotype"]))
  colnames(tmp) <- c('pheno',person)
  data_pheno_stat_subset <- merge(x=data_pheno_stat_subset,y=tmp,all.x=TRUE)
}
