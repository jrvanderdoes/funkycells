...
organizeTNBCs <- function(CDir) {
  ...
  for (i in 1:N) {
    ...
    temp_xy <- pixel_xy[[i]]
    temp_val <- lapply(
      tnbc$cellLabelInImage[temp_idx],
...
generateTNBCs <- function(CDir) {
  ...
  tnbc41_xy_read_fact_class <- read.csv(
    paste0(CDir, "generated/", "cellData_xy_Class.csv")
  )[, -1]
  tnbc41_xy_class <- read.csv(
    paste0(CDir, "generated/", "cellData_xy_Class_int.csv")
  )[, -1]
...
cleanTNBC <- function(TNBC) {
  TNBC[, c(
    "Class", "Person", "cellLabelInImage", "cellx", "celly",
...
#' Get PCA to Save
#'
#' Converts TNBC data to PCA data with meta for use in package and minimal size.
#'
#' @param data Data.frame of cell data
